...
$(document).on('knack-form-submit.view_370', function(event, view, data) { 
  ...
  "Damage Photo 4":data.field_720_raw, "Damage Photo 5":data.field_719_raw, "Damage Photo 6":data.field_721_raw, "Side Profile Photo":data.field_5372_raw, "Date Of Last Service":data.field_535_raw.date_formatted, "Total Refurb Cost": "£" + data.field_624_raw,
  "Mechanical Refub Cost":"£" + data.field_622_raw, "Aesthetic Refub Cost":["£", data.field_623_raw].join(), "Valuation Notes":data.field_4390_raw, "Vehicle Test Driven":data.field_745_raw, "Offer valid Up to":data.field_3203_raw, 
  "Sales Advisor Refurb Description":data.field_882_raw + " " + data.field_883_raw, "Source Of Payload":"knack direct"};
  
  
  let newJSON = JSON.stringify(dataToSend, function (key, value) {return (value === undefined || value === null) ? "1" : value;});
