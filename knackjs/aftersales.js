      ...
      for (i = 1; i < rows.length; i++) {
        ...
        const createClickHandler = function(row) {
          return function() {
            var cell = row.id;
            console.log('cell',cell);
            callPostHttpRequest("https://hook.eu1.make.celonis.com/4w3cn2lcxhem6tp9l7dfbtc9r1sc8h6g", {"RecordID from Jobcard":data.id, "Service Comments":data.field_2203, "userName": Knack.getUserAttributes().name,"Manual Request":"Yes" },"");
          };
        };
