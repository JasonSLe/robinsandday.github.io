...
$(document).on('knack-view-render.view_1901', function(event, view) {
  //get the vin value from the table
 ...
 // trigger a webhook from a action link - Aftersales - update live individual wip from Reg & Status Lookup for Vehicles Onsite
    ...
    if ($('div[class="kn-view kn-table view_1901"]')){
      let rows = $('div[class="kn-view kn-table view_1901"] table tr');
      for (i = 1; i < rows.length; i++) {
        let currentRow = rows[i];
        const createClickHandler = function(row) {
          return function() {
